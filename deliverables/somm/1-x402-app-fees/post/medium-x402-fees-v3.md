...
*Draft v3 — pricing numbers corrected after the fee change; this is the
...
- `cost/turn = 5` — each turn draws the balance down *(confirmed with eng 07-23:
  5 units/turn is current; the 07-22 fee change did not touch the per-turn rate,
  it moved the partner split from 70/30 to 80/20)*
...
app fee, and the partner's cut each ride their own leg of the route — at the
new **80/20 app/partner split**. Because settlement happens at checkpoints, the
split is computed once per gate — not re-derived per request — which is what
keeps partner accounting exact instead of rounded.
...
*Companion visual: `fee-flow-diagram.html` in this workspace. X-thread derived
from this draft: `x-thread-x402-v1.md`.*
